...
def Phi(state):
    ...
    for i in power_set({0, 1, 2}):
        if i != set() and i != {0, 1, 2}:
            Distance = [ConceptStructure(i, state[0]), f"The MIP for state {state} is {i}"]
...
def Comparison(state1, state2):
    ...
    for i in power_set({0, 1, 2}):
        if i != set():
            ...
            Core2 = []
            for function in ['cause', 'effect']:
                Core.append(corecause2(i, state1[0], function[0]))
                Core2.append(corecause2(i, state2[0], function[0]))
            concept1.append([min(Core[0][0], Core[1][0]), Core[0][3], Core[1][3], i, state1[0]])
            concept2.append([min(Core2[0][0], Core2[1][0]), Core2[0][3], Core2[1][3], i, state2[0]])
            # these take the form phi, core cause, core effect, mechanism, state
    Grep1 = 0
    for j in range(len(concept1)):
        a = concept1[j][0] * Repertoire2(concept1[j][1], concept1[j][3],
